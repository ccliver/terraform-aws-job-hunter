...
| <a name="input_location"></a> [location](#input\_location) | Comma-separated location substrings (OR'd together) to additionally keep for every ATS backend except builtin; blank disables it (remote-only). Independent of builtin\_location | `string` | `"VA,Virginia,DC"` | no |
