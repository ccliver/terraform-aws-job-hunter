...
| <a name="input_enable_clearance_filter"></a> [enable\_clearance\_filter](#input\_enable\_clearance\_filter) | Whether to drop job postings that require a security clearance above Public Trust. When false, the Workday and Built In backends also skip the extra per-posting detail-page request that exists only to feed this check | `bool` | `true` | no |
